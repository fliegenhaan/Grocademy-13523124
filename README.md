...
L - Liskov Substitution Principle:Eloquent Models: Semua model (User, Course, Module, dll.) adalah subclass dari Illuminate\Database\Eloquent\Model. Seperti di dalam CourseService, memanggil metode-metode seperti find(), create(), all(), atau save() pada model-model ini. Kita dapat mengganti objek Course dengan objek Module di dalam logika yang serupa (misalnya, jika ada repository generik), dan program tidak akan rusak karena keduanya adalah turunan dari superclass Model dan memiliki "kontrak" metode yang sama.
I - Interface Segregation Principle: Prinsip ini diwujudkan melalui penggunaan interface yang spesifik dan tidak "gemuk". Contoh terbaik adalah pada model User yang mengimplementasikan interface JWTSubject. Interface ini hanya menuntut implementasi dua method yang sangat relevan untuk otentikasi JWT, yaitu getJWTIdentifier() dan getJWTCustomClaims(), sehingga model User tidak dipaksa untuk membawa beban method yang tidak diperlukannya.
